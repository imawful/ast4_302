// Name: Kevin Barrios
// NSHE ID: 2001697903
// Assignment 4 word classification
//  use maps in c++ to search elements in constant time
#include <iostream>
#include <vector>
#include <string>
...
int main()
{
	...
	for(int testCase = 0; testCase < noAbstracts; testCase++)
	{
		...
		std::unordered_map<std::string, KeyWord> Daisy;
			...
			else
			{
				...
				for(int i = 0; i < n; i++)
				{
					if(i == (n-1)) // last character of the line
					{
						...
						//TIME TO SEARCH 
						if(Daisy.find(tempWord) != Daisy.end() && Daisy.at(tempWord).seen == false)
						{
							WordsFound[Daisy.at(tempWord).cat] += 1;
							Daisy.at(tempWord).seen = true;
						}
